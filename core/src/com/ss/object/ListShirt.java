...
public class ListShirt {
    ...
    public ListShirt(TextureAtlas atlas, Group group, GamePlay gamePlay,TextureAtlas uiAtlas,BitmapFont font){
        this.font = font;
        this.group = group;
        ...
        this.atlas = atlas;
        if(GMain.Pref.getInteger("check4")==0){
            setArrLock();
            GMain.Pref.putInteger("check4",1);
            GMain.Pref.flush();
        }else {
            System.out.println("check:"+GMain.Pref.getString("shirt"));
            String str = GMain.Pref.getString("shirt").replace("[","");
            str = str.replace("]","");
            str2  =str.split(",");
        }
        loadListItem();
    ...
    private void setArrLock(){
        ...
        GMain.Pref.putString("shirt",arrSet.toString());
        GMain.Pref.flush();
        System.out.println("check:"+GMain.Pref.getString("shirt"));
        String str = GMain.Pref.getString("shirt").replace("[","");
        str = str.replace("]","");
    ...
    private void loadListItem(){
        ...
        for (int i=0;i<10;i++){
            item = GUI.createImage(atlas,"ao"+(i+1));
            item.setWidth(item.getWidth()*(Config.ratioX-0.4f));
            item.setHeight(item.getHeight()*(Config.ratioX-0.4f));
            item.setOrigin(Align.center);
            item.setPosition((paddingX*0.6f)*i,-120);
    ...
    private void Unlock(int index){
        ...
        GMain.Pref.putString("shirt",arrStr.toString());
        GMain.Pref.flush();
