...
public class ListDress {
    ...
    public ListDress(TextureAtlas atlas, Group group, GamePlay gamePlay,TextureAtlas uiAtlas,BitmapFont font){
        this.font = font;
        this.group = group;
        ...
        this.atlas = atlas;
        if(GMain.Pref.getInteger("check3")==0){
            setArrLock();
            GMain.Pref.putInteger("check3",1);
            GMain.Pref.flush();
        }else {
            System.out.println("check:"+GMain.Pref.getString("dress"));
            String str = GMain.Pref.getString("dress").replace("[","");
            str = str.replace("]","");
            str2  =str.split(",");
        }
        loadListItem();
    ...
    private void setArrLock(){
        ...
        GMain.Pref.putString("dress",arrSet.toString());
        GMain.Pref.flush();
        System.out.println("check:"+GMain.Pref.getString("dress"));
        String str = GMain.Pref.getString("dress").replace("[","");
        str = str.replace("]","");
    ...
    private void loadListItem(){
        ...
        for (int i=0;i<10;i++){
            item = GUI.createImage(atlas,"vay"+(i+1));
            item.setWidth(item.getWidth()*(Config.ratioX-0.6f));
            item.setHeight(item.getHeight()*(Config.ratioX-0.6f));
            item.setOrigin(Align.center);
            item.setPosition(50+(paddingX*0.6f)*i,-100);
            ...
            Lock.setPosition(150+(paddingX*0.6f)*i,100);
    ...
    private void setTouch(boolean set){
        for(int i=0;i<arrItem.size;i++){
            if(set==false){
    ...
    private void Unlock(int index){
        ...
        GMain.Pref.putString("dress",arrStr.toString());
        GMain.Pref.flush();
