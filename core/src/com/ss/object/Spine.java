...
public class Spine extends Actor {
    ...
    public Spine(){
        ...
        atlas = new TextureAtlas(Gdx.files.internal("spine/girl.atlas"));
        SkeletonJson json = new SkeletonJson(atlas);

        json.setScale(0.5f); // Load the skeleton at 60% the size it was in Spine.
        SkeletonData skeletonData = json.readSkeletonData(Gdx.files.internal("spine/girl.json"));

        skeleton = new Skeleton(skeletonData); // Skeleton holds skeleton state (bone positions, slot attachments, etc).
        skeleton.setPosition(GStage.getWorldWidth()/2, skeleton.getData().getHeight()+120);
        skeleton.setScale(3f*Config.ratioX,-3f*Config.ratioX);
        //skeleton.set
